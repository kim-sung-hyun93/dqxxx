...
/*
<<상속관계에서 멤버변수가 갖는 특징>>
1. 상속관계에서는 상위클래스에서 정의된 멤버변수를 하위클래스에서 사용할 수 있다.(하위 클래스 참조 변수를 통해서 접근할 수 있다.)
2. 상위클래스의 변수와 동일한 이름의 변수를 하위클래스에 정의하는 경우 하위클래스의 멤버변수가 우선순위가 높다.
3. 부모클래스의 변수를 액세스 하려면 super를 이용해서 접근한다.	
	this -> 자기자신의 객체
	super -> 부모객체
4. private로 선언된 변수는 상속관계에 있다고 하더라도 하위클래스에서 접근할 수 없다.
*/
...
public class InheritanceTest01 {
	public static void main(String[] args) {
		...
		//상위클래스를 쓰는게 아니라 하위클래스를 쓰는것이다.
		Sub obj = new Sub(); //obj의 정의로 하위클래스인 Sub클래스를 참조해서 사용할 수 있다.
		obj.display();
